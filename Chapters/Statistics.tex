...
\begin{mybox}{Shannon's information entropy}
	...
	H(X)=\expval{i(x)} = - \tr_{\mx} p(\mx) \ln p(\mx) = S_p,
	\ee 
...
\begin{enumerate}
	...
	One has 
	\bse 
	\frac{\partial \mL}{\partial p(x)} = -\ln p(x) -1 +\lambda =0 \; \Rightarrow \; p(x) = e^{\lambda-1 }.
	\ese 
...
D_{KL}(p||q) = \text{Tr}_{\mx}p(\mx) \log \frac{p(\mx)}{q(\mx)} = \sum_x p(\mx) \left[\ln p(\mx) - \ln q(\mx)\right],
\ee 
...
\bse 
p(x_m) \rightarrow \mathcal{P}(x) \md x, \quad q(x_m) \rightarrow \mathcal{Q}(x) \md x,
\ese  
with \emph{probability densities} $\mathcal{P}(x), \mathcal{Q}(x)$. The relative entropy becomes
\be 
D(p||q) = \int \md x \mathcal{P}(x) [\ln(\mathcal{P}(x) \md x) - \ln (\mathcal{Q}(x) \md x) ] = \int \md x \mathcal{P}(x) [\ln\mathcal{P}(x) -\ln \mathcal{Q}(x)].
\ee 
In contrast, Shannon's entropy \ref{eq:infoShannon} has the formal continuum limit
\be 
H(X) \rightarrow \int \md x \mathcal{P}(x) \ln (\mathcal{P}(x) \md x),
\ee 
...
\bse 
D(q+\delta q || q) = \sum_x \frac{1}{2 q(x)} \delta q(x)^2 + \dots 
\ese 
