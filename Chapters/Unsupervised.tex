...
\subsubsection{The cost function of adversarial training}
...
\be 
\mC(G) = -\log 4 + 2 D_{JS} (p_{data},p_{\theta_G}).
\ee
...
\subsubsection{VAEs as variational methods}
...
One can write the log-likelihood s
 	...
 	\be 
 	\label{eq:vaeGANELBO}
 	-F_{q\phi}(\mx) \equiv \mathbb{E}[\log p_\theta(\mx |\mz)] - D_{KL} (q_\phi (\mz |\mx) | p(\mz)),
 	\ee 
...
\be 
\mathbb{E}_{q_\phi (\mz |\mx) } [f(\mz)] = \mathbb{E}_{p_{\mathbf{ε}}}[f(\mz)]. 
\ee 
Evaluating the derivative then becomes quite straight forward since
\be 
\nabla_\phi \mathbb{E}_{q_\phi } [f(\mz)] \sim \mathbb{E}_{p_{\mathbf{ε}}} [\nabla_\phi f(\mz)].
\ee 
Of course, when we do this we still need to be able to calculate the Jacobian of this change of variables
\be 
d_\phi (\mx,\phi) = \det \abs{\frac{\partial \mz}{\partial \mathbf{ε}}}
\ee 
since
\be 
\log q_\phi(\mz |\mx) =\log p(\mathbf{ε}) - \log d_\phi(\mx,\phi).
\ee
\end{enumerate}
Since we can calculate gradients, we can now use backpropagation on the full ELBO objective function, see below.\\
...
\bse 
\mathbb{E}_{q_\phi(\mz|\mx)} [\log p_\theta(\mx |\mz)] - \beta D_{KL}(q_\phi(\mz|\mx) |p(\mz))
\ese 
...
\bse 
q_\phi (\mz |\mx) = \mathcal{N}(\mz,\mm(\mx), \text{diag}(\mathbf{\sigma}^2(\mx) ))
\ese 
...
\bse 
\mt^* = \arg \max_{\mt} \mathbb{E}_{x \sim p_{data}} \log p_{model}(\mx |\mt).
\ese 
...
\subsubsection{Comparing GANs to other generative models}
...
\begin{figure}[h!]
	...
	\label{fig:gan1}
\end{figure}
Formally, GANs are a structured probabilistic model containing latent variables $z$ and observed variables $\mx$. 
